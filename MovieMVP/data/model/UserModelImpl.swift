...
extension UserModelImpl : UserModel {
	...
	func createSessionWithLoginFromApi(username:String,
								password:String,
								token:String,
		...
		URLSession.shared.dataTask(with: request) { (data, response, error) in
			...
			if let data = response {
				...
				self.createSessionFromApi(token: data.request_token ?? "", success: {
					success()
				}) { (error) in
					print(error)
				}
	...
	func getUserDetailFromApi(success :@escaping(LoginResponse) -> Void,
					   failure :@escaping(String) -> Void) {
